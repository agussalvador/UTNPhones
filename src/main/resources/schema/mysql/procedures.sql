...
DELIMITER $$
CREATE PROCEDURE sp_insert_user_client( pIdCity BIGINT, pFirstname VARCHAR(50), pLastname VARCHAR(50), pDni VARCHAR(20), pPwd VARCHAR(128), pTypeLine ENUM('mobile','home') ,OUT pIdUser BIGINT )
BEGIN
...
DELIMITER $$
CREATE PROCEDURE sp_update_user (pId_city int, pFirstame VARCHAR(50), pLastname VARCHAR(50), pDni VARCHAR(20), OUT pIdUser BIGINT)
BEGIN
	UPDATE users 
...
DELIMITER $$
CREATE PROCEDURE sp_insert_tariff (pId_city_origin BIGINT, pId_city_destination BIGINT, pCost_price DOUBLE, pPrice DOUBLE, OUT pIdTariff BIGINT )
BEGIN
    START TRANSACTION;
...
call sp_insert_call('02205882727','00115634979',45,now(),@idCall);
